...
The scalar plot's **Share** button POSTs the rendered figure (`plotDiv.data` + `layout`) to `create_shared_plot`, which stores it in `SharedPlot` under a random token and returns `/p/<token>`. That link is a **snapshot** — interactive (zoom/pan/hover/legend), but the data is frozen at share time and survives the source being deleted. `/p/<token>` is standalone (`shared_plot_view.html`, no base template) and listed in `load_project_context`'s `public_endpoints`, because recipients have no project cookie and would otherwise be redirected to `/projects`. **Shared Plots** in the project navbar (`list_shared_plots`) shows views / last viewed and deletes links; a deleted token renders a 404 "no longer available" page. Clicking a point, in the app or on a shared plot, opens the comparison for **only that point's submission** (`compare_ids`), filtered to that sample (`search_query`, `per_page=1`). Sample traces carry `meta.pointSubs`, aligned with x/y/text, because a cluster series pools several submissions. Mean/Std traces keep theirs in `meta.raw.sub`. Points with no submission (ground truth vs ground truth) keep the full comparison. `SharedPlot.source_url` is stored as a **path + query only**, via `_shared_plot_source_path()`, and the page resolves it against the viewer's own origin. The sharer's browser URL often names a host only they can reach, such as localhost. Anything that is not a local path is dropped. The shared page also has an **Aggregation** control (None / Mean / Std, default None), which re-aggregates the snapshot in the browser. For this to work, Mean/Std traces in the comparison plot carry their samples in `meta: {aggKind, raw: {x, y, text}}`; raw scatter/line traces already hold theirs. **Show** toggles, one per Y **field**, are available in the app and on the shared page. `visKeyOf()` maps every `sub::<id>::<name>` to `all::<name>`, so one switch covers that field in every submission and cluster, while `gt::`/`tag::` fields keep their own switch. Single series are toggled from the legend. With two or more fields, a **Multiple / Toggle** switch sits in the Show row. Toggle shows one field at a time using radio buttons, and hidden fields get `visible: false` so they drop out of the legend. In Toggle mode, legend entries name only the submission (`subalias::<id>` alias, else the submission label) or the cluster (`cluster::*||<value>`), with one alias covering every field, and each submission keeps its color across fields. `layout.meta.yAxis.showMode` carries the mode to the shared page. Each field can also have its own **Y axis label**, set in the Labels panel. It is stored in the alias map as `yaxis::<visKey>`, and the Y title joins the labels of the fields currently shown. The whole-axis label overrides all of them. Traces carry `meta.series.axisLabel` and the layout carries `meta.yAxis = {override, fn}`, so the shared page can rebuild the title when fields are toggled or the aggregation changes. A hidden series is drawn `visible: 'legendonly'` and is excluded from auto-zoom. Each trace carries `meta.series = {key, label}`, from which the shared page rebuilds its toggles, starting from the visibility at share time. Box traces, and Mean/Std shares made before `meta.raw` existed, are drawn exactly as shared (the control is hidden when no trace has samples). The table is created by `db.create_all()` in `bootstrap()`. Like everything else here there is no auth: the token is the only access control.
...
- The same applies to `edit_leaderboard` (the metric editor page): it needs only field *names*, so it uses `DISTINCT` queries over `(submission_id, name, field_type)` and never loads CustomField rows. At 128 submissions x 3.6k rows each, loading the rows took 10s; the DISTINCT queries take 0.2s. `CustomField` has indexes on `(submission_id, name)` and `(sample_id)`, declared in `__table_args__` and created for existing DBs by `check_and_migrate_db()`.
- On the metric/visualization pages, the server prefers an attached file over the code textarea. The **Load** button therefore clears the file input after it copies the file into the editor. Otherwise edits made after loading are silently replaced by the file's content on save.
...
- Raw histogram counts (`gt_hist`, `sub_hist_<folder>`, `gt_hist_<folder>`) are **opt-in**: the builders keep the arrays only when the key appears in `needed_keys`, which callers derive with `mapped_context_keys(leaderboard.leaderboard_metrics)`. The data is parsed regardless to produce the entropies, so this costs no extra I/O — it decides whether hundreds of arrays are retained. Everything else in the context is a float.
- `gt_*` context keys come from **dataset rows only** (`CustomField.submission_id IS NULL`). Submission rows also carry `sample_id`, so any query that matches on `sample_id` alone sweeps in every submission's fields — that used to leak into `gt_*` (last row by id winning) and into the Dataset suggestion list. Filter on `submission_id` whenever "ground truth" is meant. With a GT-source submission, `gt_<name>` is that submission's field `<name>`.
- Mapping rows add the prefix from their source (`gt_` / `sub_`), so suggestion lists must hold bare names — never pre-prefixed ones, which save as `sub_sub_…`.
